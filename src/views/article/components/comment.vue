<template>
  <div class="comment">
    ...
    <van-action-sheet v-model="showReply" :round="false" class="reply_dialog" title="回复评论">
        <!-- :immediate-check="false"关闭自动执行load事件 -->
  ...
  data () {
    return {
      ...
      reply: {
        ...
        list: [], // 存放评论的评论的数据
        commentId: null // 存放评论id   用来查询这个评论的评论
      }
    }
  },
  methods: {
    ...
    openReply (commentId) {
      ...
      // 先把之前的数据清空  相当于重置数据
      this.reply.list = []
      this.reply.offset = null // 点击弹出回复面板的时候   是新数据  从第一页开始
      this.reply.finished = false // 将finished打开
      this.reply.loading = true // 主动打开加载状态   因为此时没有主动检查
      // 弹出评论时  主动请求一次数据
      this.getReply()
    },
    // 此方法用来获取评论的评论  会在第一次执行  加载第二次第三次。。。时也执行
    async getReply () {
